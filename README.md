...
```bash
uv tool install specify-cli --from git+https://github.com/demonyangyue/spec-kit-pro.git
```
...
```bash
uv tool install specify-cli --force --from git+https://github.com/demonyangyue/spec-kit-pro.git
```
...
```bash
uvx --from git+https://github.com/demonyangyue/spec-kit-pro.git specify init <PROJECT_NAME>
```
...
`specify init` creates or extends `.specify/` with memory (product, architecture, norms, common), `settings/rules`, and `AGENTS.md`. Launch your AI assistant in the project directory. The `/speckit.*` commands are available in the assistant.

Use the **`/speckit.constitution`** command to create your project's governing principles and development guidelines that will guide all subsequent development. On first run, if the project has no knowledge-base docs yet (e.g. under `.specify/memory/product/` or `.specify/memory/architecture/`), the command can also generate a product overview and tech-stack baseline and update `.specify/memory/AGENTS.md`.
