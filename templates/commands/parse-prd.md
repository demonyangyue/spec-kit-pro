...
- 用户回复后，将所选答案写回 结构化需求.md 中对应位置，并可根据需要再次执行步骤 6 质量校验
...
### 6. 需求质量校验（Requirements Quality Validation）
...
- [ ] 关键 [待确认] 已通过步骤 4 澄清闭环，或已明确标注为可选/备选
...
- 面向业务与产品方，而非开发实现细节
- 澄清仅在步骤 4 进行；数量与优先级明确（范围 > 安全/合规 > 体验 > 技术细节）
...
3. **质量校验结果**：步骤 6 自检通过/不通过；若不通过，列出剩余问题及建议
